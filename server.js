...
${bullets}`;
  ...
  try {
    ...
    if (!res.ok) {
      categorySummaryStatus[categoryKey] = `Groq ha risposto con errore ${res.status}`;
    ...
    if (result) {
      ...
      categorySummaryStatus[categoryKey] =
        `scartato — Groq ha risposto ${raw.length} caratteri (finish_reason: ${finishReason}), ` +
        `ma non ha superato i controlli di qualità. Testo grezzo: "${raw.slice(0, 150)}${raw.length > 150 ? '...' : ''}"`;
    }
    return result;
  } catch (err) {
    if (!categorySummaryStatus[categoryKey]) categorySummaryStatus[categoryKey] = `errore: ${err.message}`;
...
async function generateWeeklySummaries() {
  ...
  for (const cat of Object.keys(CATEGORY_LABELS)) {
    ...
    newWeekly[cat] = { text: await summarizeWeek(cat, days), daysCovered: days.length };
  }
